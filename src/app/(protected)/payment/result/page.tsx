...
import { initiateRefund, checkRefundStatus, verifyPaymentStatus } from "@/lib/services/payment";
...
export default function PaymentResultPage() {
  ...
  const {
    ...
    queryFn: async (): Promise<PaymentData> => {
      const res: ApiResponse = await verifyPaymentStatus({ orderId, gateway: "phonepe" });
      return res.data.data.payment;
    },
    enabled: !!orderId,
    refetchInterval: (query) => {
      const data = query.state.data;
      // If there's an active refund that's not terminal, poll every 3 seconds
  ...
  const refundMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      // Trigger immediate refetch and enable polling
      refetchPayment();
    },
    onError: (err: any) => {
      const message = err?.response?.data?.message || err?.message || "Refund failed";
      toast.error(message);
    },
  });
  ...
  const manualRefundCheck = useMutation({
    mutationFn: async () => {
      if (!payment?.refund?.refundId) {
        throw new Error("No refund ID found");
      }
    ...
    onSuccess: (data) => {
      ...
      refetchPayment(); // Refresh payment data to get updated refund status
    },
    onError: (err: any) => {
      const message = err?.response?.data?.message || err?.message || "Failed to check refund status";
      toast.error(message);
    },
  });
  ...
  const handleInitiateRefund = () => {
    ...
    const confirmed = confirm(
      `Refund ${formatCurrency(payment.amount, payment.currency)}? This cannot be undone.`
    );
  ...
  const getStatusIcon = (state: string) => {
    switch (state?.toUpperCase()) {
      case "COMPLETED":
      case "SUCCESS":
        return <div className="w-20 mb-4">
          <Lottie animationData={successAnimation} loop={false} />
        </div>;
      case "FAILED":
        return <div className="w-80 mb-4">
    ...
    return (
      <div className="flex justify-center items-center min-h-[70vh] px-4">
        <Card className="max-w-md w-full p-8 text-center">
          ...
          <p className="text-gray-600 mb-6">Please check your order reference and try again.</p>
